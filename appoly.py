...
st.set_page_config(page_title="Scanner Confluence Forex (Polygon)", page_icon="⭐", layout="wide")
...
if not POLYGON_API_KEY: 
    st.error("Clé API Polygon.io non disponible après la lecture des secrets.")
    st.stop() 
else:
...
def smoothed_heiken_ashi_pine(dfo,l1=10,l2=10):
    ...
    try:
        ...
        sho=ema(hao_i,l2);shc=ema(hac_i,l2)
        return sho,shc
    except Exception as e: 
...
def ichimoku_pine_signal(df_high, df_low, df_close, tenkan_p=9, kijun_p=26, senkou_b_p=52):
    ...
    try:
        ...
        ctn=max(cssa,cssb);cbn=min(cssa,cssb)
        sig=0
        if ccl>ctn:
            sig=1
        elif ccl<cbn:
...
def get_data_polygon(symbol_pg: str, timespan_pg: str = 'hour', multiplier_pg: int = 1, days_history_pg: int = 30):
    ...
    if POLYGON_API_KEY is None: 
        st.error("FATAL: Clé API Polygon non chargée.")
        print("FATAL: Clé API Polygon non chargée.")
        return None
    ...
    try:
        ...
        if response is not None: 
            print(f"Réponse Polygon: {response.text}")
        return None
    except Exception as e:
        st.error(f"Erreur inattendue get_data_polygon pour {symbol_pg}: {type(e).__name__}")
        st.exception(e)
        print(f"ERREUR INATTENDUE get_data_polygon {symbol_pg}:\n{traceback.format_exc()}")
        return None

# --- Fonction calculate_all_signals_pine (CORRIGÉE) ---
